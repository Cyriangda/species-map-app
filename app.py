...
st.subheader(f"Click on a country to see its species with status '{selected_status}'")
clicked = st.plotly_chart(fig, use_container_width=True)

# ===============================
# 🌍 Récupérer le pays cliqué
# ===============================
country_clicked = st.session_state.get("country_clicked", None)

# Fonction pour stocker le clic
def store_click(trace, points, state):
    if points.point_inds:
        iso = country_counts.iloc[points.point_inds[0]]["ISO3"]
        st.session_state.country_clicked = iso

# ===============================
# 🧬 Liste des espèces pour le pays cliqué
# ===============================
if country_clicked:
    species_in_country = df_filtered[df_filtered["ISO3"] == country_clicked]
    ...
    st.subheader(f"Species in {country_clicked} with status '{selected_status}'")
    ...
    if species_in_country.empty:
        ...
        for _, row in species_in_country.iterrows():
            ...
            with st.expander(f"{full_name} ({english_name})"):
                ...
                st.markdown(f"**ID :** {row['species_id']}")
else:
